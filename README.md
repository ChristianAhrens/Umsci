...
|GitHub Actions CI build status|Umsci|
|-|-|
...
- **Indicator visualization** — *Dots & Line* (default) draws a labelled circle per channel joined by a thin line.  *Solid Bar* instead draws a single uniform-width bar (as wide as the dots in Dots & Line mode) along the same ring/rectangle path, with no distinct per-channel markers — including the LFE dot.  Independent of **Indicator shape**: both Circle and Rectangle work with either visualization.
